...
export default function AssetDetailsPage() {
  ...
  const pegClientId = searchParams.get('pegClientId') || '';
  ...
  useEffect(() => {
    if (currentAsset && isEditMode) {
      setFormData({
        ...
        pegClientId: currentAsset.pegClientId || null,
      });
    } else if (!isEditMode && pegClientId) {
      // Set pegClientId for new assets when coming from PEG page
      // Also set status to 'assigned' since it's being assigned to a client
      setFormData(prev => ({
        ...prev,
        pegClientId: pegClientId || null,
        status: 'assigned',
  ...
  }, [currentAsset, isEditMode, pegClientId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode && id) {
        ...
        // Navigate back to the appropriate page based on pegClientId
        if (pegClientId) {
          navigate('/assets/peg');
        } else {
          // Check returnTo query param or default to assets page
