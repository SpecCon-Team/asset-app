import{u as Q,r,f as p,j as e,Q as o,P as i,D as R}from"./index-CLEVzMqI.js";import{showError as c,showSuccess as u}from"./sweetalert-BCvQYi5m.js";import{A}from"./arrow-left-DJvAILUt.js";import{C as S}from"./circle-check-big-CZwGDxaB.js";function P(){const y=Q(),[x,g]=r.useState(!1),[b,f]=r.useState([]),[l,m]=r.useState(""),[s,h]=r.useState(null);r.useEffect(()=>{j()},[]);const j=async()=>{try{const a=await p().get("/assets"),d=Array.isArray(a.data)?a.data:a.data.assets||[];f(d)}catch(t){console.error("Failed to fetch assets:",t),await c("Error","Failed to load assets")}},w=async t=>{var a,d;if(t.preventDefault(),!l){await c("Error","Please select an asset");return}try{g(!0);const C=await p().post("/checkout/qr/generate",{assetId:l});h(C.data.qrCode),await u("Success!","QR code generated successfully",1500)}catch(n){console.error("Failed to generate QR code:",n),await c("Error",((d=(a=n.response)==null?void 0:a.data)==null?void 0:d.message)||"Failed to generate QR code")}finally{g(!1)}},k=()=>{if(!s)return;const t=document.createElement("a");t.href=s.qrCodeUrl,t.download=`QR_${s.asset.asset_code}.png`,document.body.appendChild(t),t.click(),document.body.removeChild(t),u("Downloaded!","QR code image downloaded",1500)},N=()=>{if(!s)return;const t=window.open("","_blank");t&&(t.document.write(`
        <!DOCTYPE html>
        <html>
          <head>
